-- Staging: Bihar UDISE schools. 1 row per school. Cleans the raw varchar
-- extract into a typed, readable contract. Cleaning only; no business logic.

with source as (
    select * from {{ source('udise', 'udise_schools_bihar') }}
)
...
from source
-- keep only rows with a usable school code and sane coordinates
